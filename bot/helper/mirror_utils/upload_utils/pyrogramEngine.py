    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            if self.__as_doc or force_document or (not is_video and not is_audio and not is_image):
                ...
                nrml_media = await self.__client.send_document(filename,chat_id=self.__sent_msg.chat.id,
                ...
                if self.__is_cancelled:
                    return
                buttons = await self.__buttons(self.__up_path, is_video)
                nrml_media = await self.__client.send_video(filename=self.name,chat_id=self.__sent_msg.chat.id,
                                                                    reply_to_message_id=self.__sent_msg.id,
                ...
                if self.__is_cancelled:
                    return
                self.__sent_msg = await self.__client.send_audio(filename=self.name,chat_id=self.__sent_msg.chat.id,
                                                                    reply_to_message_id=self.__sent_msg.id,
                ...
                if self.__is_cancelled:
                    return
                self.__sent_msg = await self.__client.send_photo(filename=self.name,chat_id=self.__sent_msg.chat.id,
                                                                    reply_to_message_id=self.__sent_msg.id,
